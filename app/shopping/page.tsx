...
/** Build a simple shopping list from today's daily meals ingredients */
function buildTodayShoppingItems(meals: DailyMeals): ShoppingItem[] {
  ...
  for (const type of types) {
    ...
    for (const ing of meal.ingredients) {
      ...
      if (!seen.has(key)) {
        seen.add(key)
        items.push({ name: ing, category: 'other', checked: false })
      }
    }
  }
  return items
...
export default function ShoppingPage() {
  ...
  // If we have a weekly plan, show that (Pro feature)
  if (plan) {
    return (
  ...
  // Free user with today's meals — show today's ingredients
  if (todayMeals) {
    ...
    return (
      <div className="py-6 space-y-5">
        ...
        >
          ...
          <Link
            href="/meal-plan"
            className="shrink-0 text-[11px] font-bold bg-white text-[#7F77DD] px-3 py-1.5 rounded-xl"
          >
            {lang === 'zh' ? '升級' : 'Upgrade'}
          </Link>
        </div>
  ...
  // No meals at all — original empty state
  return (
    <div className="py-6 space-y-5">
      <div>
        <div className="flex items-center gap-2 mb-1">
